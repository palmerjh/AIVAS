...
class Template(object):
	...
	def createVisualizer(self):
		factor = float(visual_h0) / self.h0 	# factor by which retina must be scaled for visualization

		retina_radius = self.R0*math.pow(self.ratio,self.nRings-1) + self.h0*math.pow(self.ratio,self.nRings-1)
		...
		visual_retina_radius = factor * retina_radius
		...
		for ring in xrange(-1,self.nRings):
			radius = factor * self.R0*math.pow(self.ratio,ring + self.radius_split)
		...
		for x in xrange(visual_size):
			for y in xrange(visual_size):
				if np.array_equal(visual_pix[x,y],[255,0,0]):		# don't color in the pixels that were colored red from before
					continue
