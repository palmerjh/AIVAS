# NOTES: difference between retina_efficient and retina_subpixel:
#	- uses subset of rings for ringwiseSumProd()
#	- uses SumProd of ring-based values for ringwiseSumProd()
#	- default_hn = 2
...
density_ring_proportion = 0.3	# proportion of rings to use for density delta calculation
...
class Template(object):
	...
	def createRetinaGeneratorAndDensityCalculator(self):
		...
		def calcDensity(retina=None,pixels=None):
			...
			if pixels is None:	# assert: primary/secondary base region density being calculated --> determine subset of rings
				ring_densities = [(densities[ring],ring - 1) for ring in xrange(1,len(densities))]
				ring_densities.sort(reverse=True)

				density, prev_ring = ring_densities[0]
				h_prev_ring = self.hn*math.pow(self.ratio,self.nRings-1-prev_ring)

				used_rings = [prev_ring]

				# upperbound on number of rings around prev_ring that has information redundant to prev_ring
				# goal: avoid selecting rings all from the same region with identical pixel info
				avoidance_delta = int(math.ceil(1.0 / h_prev_ring))

				nRings_subset = int(math.ceil(density_ring_proportion * self.nRings))
				for i in xrange(1,nRings_subset):
					next_avoidance_delta = 0
					for density, ring in ring_densities:
						if ring in used_rings:
							continue
						if abs(ring - prev_ring) < avoidance_delta:
							continue
						used_rings.append(ring)

						h_ring = self.hn*math.pow(self.ratio,self.nRings-1-ring)
						next_avoidance_delta = int(math.ceil(1.0 / h_ring))
						prev_ring = ring
						break

					avoidance_delta = next_avoidance_delta

				retina.density_mappers = [density_mappers[0]] + [density_mappers[ring+1] for ring in used_rings]
				retina.densities = [densities[0]] + [densities[ring+1] for ring in used_rings]
				retina.density_wts = np.array([density_wts[0]] + [density_wts[ring+1] for ring in used_rings])

				retina.density_wts /= float(sum(retina.density_wts))
...
class Retina(object):
	...
	# used in calculating self.ringwiseSumProd(pix2)
	...
	# calculates the SumProd of the bs & ring regions of self.pix based on self.density_wts
	def ringwiseSumProd(self,pix2):
		...
		summation = self.densities + pix2_densities
		product = self.densities * pix2_densities

		return 2 * np.dot(product,self.density_wts) / np.dot(summation,self.density_wts)
...
def file2pix(file):
	...
	return 1.0 - np.asarray(im) / 255.0
