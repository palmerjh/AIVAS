...
choices_dir = 'choices'
targets_dir = 'targets_robust'
results_dir = 'efficient_ret_run1'
slice_index = 2
...
def main():
    ...
    for target in os.listdir(targets_dir):
        ...
        with open(target_outfile_path + '\\console_readout.txt', 'w') as f:
            ...
            slicePics = [tp.TaskPic(path + '\\%s' % s) for s in os.listdir(path)]
    
            slices = [r.TargetSlice(s.pix,target,size=search_size) for s in slicePics]
            s = slices[slice_index]   #********** TODO this is arbitrary; should be traditionally densest slice
            print('...DONE!\n')
            ...
            drawBoxes(sliceIm,boxes,os.getcwd()+'\\%s\\%s' % (results_dir,target),'slice_%d_regions.jpg' % slice_index)
...
def primarySearch(choices,s,locations):
    ...
    counter = 0
    for c_index in xrange(len(choices)):
...
def secondarySearch(choiceSpace,s,primary_loc,primary_wedge_index):
    ...
    for cs_loc in choiceSpace_locs:
        ...
        if x == s.size and y == s.size:
            density_deltas.append((s.secondary.ret.ringwiseSumProd(subSpace),cs_loc))

    if len(density_deltas) == 0:
        return (0,(0,0))

    density_deltas.sort(reverse=True)
...
def rotational_search(rot_ret,goal_ret):
    ...
    for wedge_index in xrange(rot_ret.template.nWedges):
    ...
    for c in choices:
        ...
        for sx in xrange(cx-size+1):
            for sy in xrange(cy-size+1):
                density_delta = rot_ret.ringwiseSumProd(c.pix[sx:sx+size,sy:sy+size])
                res = density_delta
...
def writeSubResults(choices,s,result_type,f):
    ...
    for c_index in xrange(len(choices)):
        directory = os.getcwd()+'\\%s\\%s\\choice%d\\%s' % (results_dir,s.t_name,c_index,result_type)
        ...
        p_ret.visualize(directory + '\\primary_target.jpg')
        px, py = res['p'].start
        p_subspace = choices[c_index].pix[px:px+s.size,py:py+s.size]
        s.template.createRetina(p_subspace).visualize(directory + '\\primary_choice.jpg')
        ...
        s_ret.visualize(directory + '\\secondary_target.jpg')
        sx, sy = res['s'].start
        s_subspace = choices[c_index].pix[sx:sx+s.size,sy:sy+s.size]
        s.template.createRetina(s_subspace).visualize(directory + '\\secondary_choice.jpg')
